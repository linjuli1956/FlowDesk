...
| 文件路径 | 行数 | 拆分建议 | 优先级 |
|---------|------|----------|--------|
| `src/flowdesk/ui/main_window.py` | **687行** | 拆分为主窗口基类 + Tab管理器 | 🔴 高 |
| `src/flowdesk/ui/tabs/network_config_tab.py` | **737行** | 拆分为左侧面板 + 右侧面板 + 事件处理器 | 🔴 高 |
...
├── main_window.py               # 主窗口 (687行) 🚨 需拆分
├── qss/                         # QSS样式文件目录
│   └── __init__.py              # QSS模块导出
├── styles/                      # 样式管理
│   ├── __init__.py              # 样式导出
│   └── color_scheme.py          # 颜色方案 (未实现)
├── tabs/                        # Tab页面组件
│   ├── __init__.py              # Tab导出 (33行)
│   └── network_config_tab.py    # 网络配置Tab (737行) 🚨 需拆分
├── dialogs/                     # 对话框组件
│   ├── __init__.py              # 对话框导出
...
### 1. 🚨 main_window.py (687行) → 拆分方案
```python
# 拆分为 3 个文件:
main_window_base.py      # 基础窗口类 (~200行)
tab_manager.py           # Tab页面管理器 (~150行)  
window_event_handler.py  # 事件处理器 (~200行)
```

### 2. 🚨 network_config_tab.py (737行) → 拆分方案
```python
# 拆分为 4 个文件:
network_config_tab_base.py    # Tab基类 (~150行)
adapter_list_panel.py         # 左侧网卡列表面板 (~200行)
ip_config_panel.py            # 右侧IP配置面板 (~200行)
network_tab_event_handler.py  # 事件处理器 (~150行)
```
